    ...
    if($pswd1 == $pswd2)
        {
            ...
            $query = mysqli_query($conn,"SELECT * FROM login_tbl WHERE user_name='$name' AND usr_password='$pswd' AND category='Admin'"); // selecting data through mysql_query()
            $rowquery = mysqli_num_rows($query);
            if ($rowquery == 1)
            {
                $query1=mysqli_query($conn,"UPDATE login_tbl SET usr_password='$pswd1' WHERE user_name='$name' AND usr_password='$pswd' AND category='Admin'");
                echo'<script>';
