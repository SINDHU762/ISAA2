    ...
    if($pswd1 == $pswd2)
        {
            ...
            $query = mysqli_query($conn,"SELECT * FROM login_tbl WHERE user_name='$o_name' AND usr_password='$pswd1' AND category='Admin'"); // selecting data through mysql_query()
            $rowquery = mysqli_num_rows($query);
            if ($rowquery == 1)
            {
                $query1=mysqli_query($conn,"UPDATE login_tbl SET user_name='$n_name' WHERE user_name='$o_name' AND usr_password='$pswd1' AND category='Admin'");
                echo'<script>';
