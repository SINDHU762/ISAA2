    ...
    <?php
        if($_POST)
        {
           ...
           include 'database_connection.php';
            ...
            $query=mysqli_query($conn,"SELECT product_name,brand_name,SUM(product_quantity),SUM(total_price) 
            FROM purchase 
            WHERE purchase_date BETWEEN '$from' AND '$to' GROUP BY product_name  ;
            ");

            $query1=mysqli_query($conn,"SELECT SUM(total_price) FROM purchase 
            WHERE purchase_date BETWEEN '$from' AND '$to'");

            $result=mysqli_fetch_array($query1);
            $p=$result['SUM(total_price)'];
            ...
            <?php
                ...
                echo '
                  <tr>
                    ...
                    <td>'.$row["SUM(product_quantity)"].'</td>
                    <td>'.$row["SUM(total_price)"].'</td>
                    </tr>
                ';
              ?>
         ...
         <p style="color:black">  <b>The total purchase amount =<?php echo $p; ?></b></p>
          <?php
        }
        ?>
</div>
